...
logging.basicConfig(filename='{}/log.txt'.format(program_folder), filemode="w", format="[%(asctime)s] %(levelname)s: %(message)s", datefmt="%m/%d/%Y %I:%M:%S %p")
...
for d in data:
	print("Name: {}      Description: {}        Rarity: {}".format(d[1], d[6], d[5]))